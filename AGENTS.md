...
- `player/GrandmaScreen.kt`：纯 Compose 展示与手势；不得直接访问网络、DAO、文件或解析 JSON。
- `player/PlayerViewModel.kt`：播放队列、双播放器池、进度与续播、重试、720P→480P 降级和自动下一部。
- `caregiver/CaregiverScreen.kt`：家属 UI；`CaregiverViewModel.kt`：家属状态与协程入口。
- `data/AppDatabase.kt`：Room Entity、DAO 和数据库；DAO 只允许 Repository 调用。
- `data/SettingsStore.kt`：DataStore 设置、PIN 摘要、同步时间和更新地址。
- `data/DramaRepository.kt`：导入、同步、过滤、占位解析、播放地址和进度保存。
- `data/Models.kt`：跨层领域模型。
- `network/BilibiliClient.kt`：B站接口、请求头、JSON 解析、分享跳转和播放地址。
...
`DramaEntity` 身份规则不可另起一套：

- 已解析内容：`id = "$bvid:$cid"`。
- 投稿占位：`id = "$bvid:0"` 且 `cid = 0`。
- 占位只在即将播放时通过 `ensureResolved` 展开为多 P/合集条目。
...
- 修改 Room Entity 或表结构时必须提升数据库版本、编写显式 Migration 并添加迁移测试。
...
按变更补充验证：推荐/过滤覆盖允许、拦截、去重和排序；WBI/解析使用固定样例；Room 添加 DAO/迁移测试；播放真机验证切换、后台恢复和结束续播；更新验证 HTTPS、大小、SHA-256、未知来源权限及同签名覆盖。
